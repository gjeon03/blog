...
25년에 접어들면서 다시 확인을 해보니 이전에 너무 비효율적으로 작업을 한것같아 개선을 해보았습니다.
...
unit_test:
  ...
  script:
    ...
    - |
      if grep "success Already up-to-date." yarn_log; then
        echo "No new packages, skipping yarn install."
      else
        echo "New packages, running yarn install."
        exit 123  # push cache
      fi
  retry: 1
  allow_failure:
...
retry를 했을 경우 무한 대기하던 이슈는 해결되었으며, 이를 통해 retry 방식으로 push하는 방법을 채택하여 잡을 여러개로 나누기 보단 직접적으로 사용하는곳에서 pull-push가 모두 이루어지도록 개선을 했습니다. 그리고 .yarn-cache를 사용하는것보다 시간이 훨씬 줄어듬에 node_modules를 캐싱하도록 했습니다.

아쉬웠던 점은, 특정 라이브러리를 추가하거나 제거할 때, pull을 통해 가져온 캐시를 기반으로 특정 라이브러리에 대한 추가 및 제거가 이루어질 것이라 예상했으나, yarn.lock과 node_modules 간에 불일치가 발생하여 전체 라이브러리를 다시 설치하는 문제가 있었습니다. 그래도 라이브러리 변경이 자주 일어나지 않기에, 크게 문제는 되지 않을 것 같습니다.
...
retry 키워드를 이용해 실패후 온전히 업로드된 cache로 재시도 했을때는 파이프라인의 상태가 성공으로 바뀌겠지 이 방법이네! 문제를 해결했다 생각하고 너무 기분이 좋았는데 장시간 대기하게 되는 이슈가 있어 실패로 끝난게 너무 아쉬웠습니다.
