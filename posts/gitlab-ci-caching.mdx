...
gitlab ci caching 최적화를 하면서 가장 많이 고민했던 지점은 “언제 cache를 pull 하고, 언제 push 해야 가장 효율적인가” 였습니다.

처음에는 불필요한 cache 업로드를 최대한 줄이고 싶다는 생각에 pull / push 시점을 엄격하게 분리하려고 했고, 그 결과 수동 버튼, 조건문, allow_failure, retry, artifacts, rules 등 여러 기능을 조합한 복잡한 구조까지 시도하게 되었습니다.

하지만 지금 와서 다시 돌아보면, cache의 pull / push 시점을 사전에 완벽하게 판별하려는 접근 자체가 CI 환경에서는 과도한 최적화였다는 생각이 듭니다.

CI 캐시는 “정확하게 통제해야 할 상태”라기보다는 있으면 이득이고, 조금 덜 최신이어도 큰 문제는 없는 보조 수단에 가깝습니다.
특히 node_modules나 yarn cache처럼 yarn install이 최종 상태를 항상 보장해주는 구조에서는, cache의 정합성을 CI 로직으로 완벽하게 보장하려는 시도가 오히려 파이프라인을 불필요하게 복잡하게 만들었습니다.

실제로 여러 실험을 거치며 느낀 점은 다음과 같습니다.

- cache push를 아끼기 위해 넣은 조건 로직들은
  → CI 설정의 가독성과 유지보수성을 빠르게 해쳤고

- pipeline status를 success로 유지하려는 시도는
  → cache라는 부가 요소에 비해 과도한 제약이 되었으며

- cache가 조금 덜 최적이더라도
  → yarn install은 결국 필요한 상태로 수렴했습니다.

그래서 지금의 관점에서는 “언제 push 해야 하는가”를 CI가 판단하게 하기보다는, main 브랜치에서만 cache를 갱신하고 MR에서는 항상 pull만 사용하며, 필요하다면 manual job으로 명시적으로 갱신하는 단순한 구조가 오히려 더 현실적인 선택이라는 결론에 도달했습니다.

이 글에서 정리한 여러 시도 중 실제로 운영 관점에서 남긴 것은 많지 않을 수도 있습니다.
하지만 그 과정에서 GitLab CI의 실행 시점과 cache 동작 방식, rules / artifacts / retry의 한계를 몸으로 이해할 수 있었고, 왜 단순한 구성이 유지보수에 더 유리한지를 직접 부딪혀서 알게 된 경험이라는 점에서 개인적으로는 의미있고 재미있는 실험이었습니다.
